...
def __ldap_set_field__(username, field, new_val):
    ...
    ldap_mod_list = ldap.modlist.modifyModlist(ldap_results, ldap_new_val)
...
def ldap_get_current_students():
    return [x[1] for x in __ldap_get_members__()[1:] if
            ldap_is_current_student(str(str(x[0]).split(",")[0]).split("=")[1])]
